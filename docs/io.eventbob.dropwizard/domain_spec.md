...
- EventDto is the sole crossing point between the HTTP wire format and the domain model; it must never be used as a domain object or passed through core routing logic
- HttpEventHandlerAdapter must implement the handler contract without modification; the core router must not and cannot distinguish a remote adapter from a local handler
- Duplicate capability names across all handler sources must cause a hard failure before the router is built; EventBobBundle is the single enforcement point for this rule
...
- All transport failures, HTTP error responses, and wire-format parse errors that occur in HttpEventHandlerAdapter must be surfaced exclusively as EventHandlingException; no HTTP or transport type may cross into the core
- Inline lifecycle holders are owned by EventBobBundle for the duration of the library's lifetime; their shutdown must occur in registration order on library teardown
- This module ships no entry point and no hard-coded handler configuration; all handler sources are provided by the importing microlith application through EventBobBundle's constructor arguments
