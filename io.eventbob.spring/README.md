...
This is the **Spring-based implementation of EventBob** - a Spring Boot adapter that provides HTTP REST endpoints for
event processing.
...
Microliths can communicate with each other by configuring remote handlers. A remote handler routes events to another
microlith via HTTP.
