...
/**
 * Lifecycle implementation for LowerHandler with Spring dependency injection.
 *
 * <p>Demonstrates Spring integration pattern. The lifecycle creates an isolated
 * ApplicationContext and retrieves the fully-wired handler from Spring.
 * </p>
 *
 * <p>Each handler gets its own isolated ApplicationContext. This ensures no bean
 * pollution between handlers and allows each handler to define its own
 * configuration without affecting others.
 * </p>
 *
 * <p>The handler and service remain POJOs with no Spring annotations. All Spring
...
public class LowerHandlerLifecycle extends HandlerLifecycle {
  private volatile LowerHandler handler;
  ...
  public synchronized void initialize(LifecycleContext context) {
    ...
    try {
      ctx.register(LowerHandlerConfiguration.class);
      ctx.refresh();
      handler = ctx.getBean(LowerHandler.class);
  ...
  static class LowerHandlerConfiguration {
    @Bean
    public LowerService lowerService() {
      return new LowerService();
    }

    @Bean
    public LowerHandler lowerHandler(LowerService lowerService) {
      return new LowerHandler(lowerService);
    }
  }
}
